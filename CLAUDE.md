...
| **Side-by-side strategy profiles** | Three paper bots run at once: `baseline` (this checkout, control), `aggressive` and `ride` (full code copies under `~/Projects/polybot-profiles/<name>/`, synced with `scripts/sync_profiles.sh`; strategy differences live ONLY in each `~/Library/LaunchAgents/com.polymarket.bot.<name>.plist` as `POLYBOT_TRADING__*` env overrides). They share one market cache via `POLYBOT_SHARED_DIR` (`data/shared/`: `markets_list.json` 90s TTL, `books/<slug>.json` 8s TTL, and `book_rate.lock`, a cross-process `/book` limiter) so the gateway sees one scanner. New knobs: `trading.allow_multiple_per_game`, `trading.league_min_edge_override`, `trading.let_it_ride_threshold`. Scoreboard: `python scripts/compare_profiles.py [--trades]`. |
| **College football + 4th profile (evening)** | `cfb` added to `bot/leagues.py` (ESPN `football/college-football` with `espn_query: groups=80&limit=400`, odds key `americanfootball_ncaaf`, 4×15 clock). Polymarket US uses its own cfb team codes (`aec-cfb-librty-coast-…`); `scripts/build_cfb_teams.py` joins each market's question ("… event Liberty vs Coastal Carolina …") to ESPN's team list and writes `configs/cfb_teams.json` (261/263 matched; `rio`, `utrgv` unmatched) — re-run weekly. `ABBR_MAP["cfb"]` and `LEAGUE_TEAM_FRAGMENTS["americanfootball_ncaaf"]` are loaded from it; `_match_abbr` now prefers an exact name then the longest fragment ("ohio" vs "ohio state"). FanDuel `ncaaf` page and Pinnacle league 880 wired; `SIGMA["cfb"]=(16, 12.5)`. `get_game_time_remaining` matches raw or normalized ESPN abbreviations. Fourth paper profile `balanced` = baseline + `allow_multiple_per_game`. |
| **Action Network + Kalshi (late evening)** | `ActionNetworkClient` (`bot/signals/book_scrapers.py`): the app's own unauthenticated JSON (`api.actionnetwork.com/web/v1/scoreboard/{nfl|ncaaf|nhl}`) gives per-book moneyline/spread/total for DraftKings (68), BetMGM (75), Caesars (123), bet365 (79), BetRivers (71); FanDuel (69) skipped (direct client is live-aware), 15 = Consensus, 30 = Open. Each book has a `game` row (pregame, frozen at kickoff) AND a `live` row refreshed in-play; scheduled games use `game`, in-progress games use `live` rows no older than `AN_LIVE_MAX_AGE` (150s), tagged live=True (25s refetch while live). In-play quotes supersede a book's pregame quote in `MultiBookAggregator`, so live NFL/CFB/NHL consensus = Pinnacle + FanDuel + DK/BetMGM/Caesars/bet365/BetRivers in-play lines. Feeds both the moneyline consensus and `LinesCache`. NYG@LAR pregame: 5 books ML, 6 books lines; college: 7 books. **Kalshi** (`bot/signals/kalshi.py`): public API (`api.elections.kalshi.com/trade-api/v2/markets`, `*_dollars` prices), series KXNFLGAME/KXNFLSPREAD/KXNFLTOTAL/KXNCAAFGAME/KXNCAAFSPREAD/KXNCAAFTOTAL/KXNHLGAME; ticker `KX…-26SEP21NYGLAR-NYG` = date + away+home + outcome; college codes are Polymarket's `displayAbbreviation` (now stored as `display_abbr` in cfb_teams.json). `kalshi_cross` is an AUX signal (weight 0.25, confidence ≤ 0.6 scaled by bid/ask width, off above 8c) on moneylines, spreads and totals. NHL abbreviation map + league-scoped fragments added. |
| **Websocket order-book feed (2026-09-21 00:xx)** | `bot/book_feed.py` `BookFeed`: background thread + asyncio, `MarketsWebSocket.subscribe_market_data(slugs)` (SDK 0.1.2, `wss://api.polymarket.us`), stores each `marketData` payload (same shape as REST `/book`, so `get_us_order_book` parses both). Order of book sources: feed (fresh < 15s) → shared cache → REST. Enabled per process by `POLYBOT_BOOK_FEED=1` (baseline plist only); the leader mirrors streamed books into `data/shared/books/` so sibling profiles get them via the cache. Subscription universe follows each full scan (`set_slugs`). **Requires a valid API key pair**: the websocket handshake is authenticated and the dead March key gets HTTP 401 → the feed logs `book_feed_disabled (auth_rejected)` and the bot keeps using REST. After generating a key at polymarket.us/developer: put it in `.env`, `launchctl kickstart -k gui/$(id -u)/com.polymarket.bot`, then `python scripts/ws_smoke.py <slug>` to confirm books stream. |
